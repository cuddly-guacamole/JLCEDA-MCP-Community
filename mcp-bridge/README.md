...
Server 通过 `bridge_recover_client` 请求受控恢复时，Bridge 会等待底层 EDA Promise 结束后再创建新运行时世代；请求本身不能取消 EDA Promise。Promise 持续挂起时需重启 EDA 宿主，然后核对目标图页并只读回读。隔离期间 Bridge 可处理只读查询，但其结果在原调用结束前只是暂时快照；写入仍被阻止。
