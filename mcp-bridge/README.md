...
`schematic_read` 在普通读取和完整连接回读前后核对图页及编辑器文档 UUID，交叉比对器件列表与当前页图元 ID，并拒绝此前已在其他图页读到的图元 ID。通过 MCP 新建、复制、打开、激活、关闭或删除图页前会记录旧页器件 ID，因此首次读取新页也能识别旧缓存。返回 `PAGE_NOT_READY` 时等图页加载后重试；成功结果包含 `pageUuid`。
