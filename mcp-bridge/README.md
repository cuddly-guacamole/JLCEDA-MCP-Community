...
`pcb_layer_manage` 以 `action:read` 读取当前 PCB 的铜层数和完整图层清单；`action:set` 接受官方支持的 2–32 偶数 `copperLayerCount`，调用前后核对同一 PCB 的铜层数。目标内层仍有图元时，EDA 可能拒绝减少铜层。写入结果不明时隔离后续写入，并以同页 `action:read` 恢复回读。
