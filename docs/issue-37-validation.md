...
本次结果说明示例中的 `nets` 字段在该客户端仍未让自动布线正常执行。因测试 PCB 与原 Issue 的 62 网络 PCB 不同，不能据此判定其参数筛选是否生效；Issue #37 继续开放。官方文档的 [autoRouting 方法示例](https://prodocs.lceda.cn/cn/api/reference/pro-api.pcb_document.autorouting.html) 使用 `nets`，锁定的 `@jlceda/pro-api-types@0.4.15` 则声明 `RoutingNets`，两者目前不一致。
