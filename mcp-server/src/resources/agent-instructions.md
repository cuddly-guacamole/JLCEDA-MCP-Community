...
- 创建或修改当前 PCB 覆铜时，使用 `pcb_pour_manage` 并提供已有网络、可用铜层及单个 `polygonSource` 轮廓数组；先 `action:read` 核对现有边框。创建和修改后按需要单独调用 `action:rebuild`，重建结果不明时不要盲目重试。恢复回读须用 `readbackPath:"/bridge/jlceda/pcb/pour-manage"` 与 `readbackPayload:{"action":"read"}` 核对原 PCB 全部边框及填充 ID、关联、数量和 `fillGeometryDigest`；诊断要求宿主重启时先重启原宿主。
