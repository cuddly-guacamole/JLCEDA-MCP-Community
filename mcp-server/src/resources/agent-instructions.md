...
- 原理图当前页器件 ID 回读可用 `api_invoke` 调用 `eda.sch_PrimitiveComponent.getAllPrimitiveId`，传 `args: []`；这个无参数调用和 `getAll` 的无参数调用可通过恢复期只读隔离。跨页查询不得用于判断当前页的超时操作是否提交。
...
- `schematic_connectivity_action`：创建导线前先用 `wire_preview` 查看当前页交点，确认意图后在 `wire_create.allowedWireIds` 中明确列出允许接触的导线 ID；写入后用 `schematic_review` 检查网表。`netport_create` 是创建同页连接/层次图端口的选项，结果会回读当前页图元和目标网络；它不等于跨页连接标识。移动已有端口用 `netport_move`，不要对 NetPort 调用仅支持普通器件的 `sch_PrimitiveComponent.modify`。
