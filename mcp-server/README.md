...
`pcb_layer_manage` 的 `read` 返回当前 PCB 铜层数量及完整图层清单；`set` 接受 2–32 的偶数 `copperLayerCount`，写后核对同一 PCB 的层数与启用的 SIGNAL/PLANE 层数量。EDA 会拒绝移除仍有图元的内层。若结果不明，使用 `bridge_recover_client action=readback`，指定 `readbackPath:"/bridge/jlceda/pcb/layer-manage"`、`readbackPayload:{"action":"read"}`；诊断要求时先重启原宿主。
