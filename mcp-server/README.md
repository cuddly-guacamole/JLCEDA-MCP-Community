...
`schematic_text_manage` 的 `read` 返回当前页全部文字标注或指定 `primitiveId` 的单条文字；`create` 至少传入 `x`、`y`、`content`，可选旋转、颜色和字体；`modify` 用 `property` 更新指定字段；`delete` 按 ID 删除。现场 EDA 3.2.181 / API 0.3.15 无法可靠写入文字 `alignMode`，本工具只回读该字段，拒绝显式设置。每次写入后核对同页目标状态。若返回 `commitUnknown:true`，使用 `bridge_recover_client action=readback`，指定 `readbackPath:"/bridge/jlceda/schematic/text-manage"`、`readbackPayload:{"action":"read"}`，完整回读原图页文字；诊断要求时先重启原 EDA 宿主。
